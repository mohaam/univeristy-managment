...
def dependson(id,prog_name):
    ...
    if request.method == 'POST':
        ...
        print(level1)
        if course[:-1] == courses_depends_on[:-1] or level1==level2:
            return render_template('dependson.html',courses=courses, result = result,alretdb="enter valid data")
        try:
            sessionDB.execute("insert into course_has_course (course_id,course_program_level,course_program_name, "
                                  ...
                                  {"course_id":course[:-1],"course_program_level":course[-1],
            ...
            return redirect(url_for('dependson',id=id,prog_name=prog_name))
        except:
            return render_template('dependson.html', courses=courses, result=result, alretdb="enter valid data")
    ...
    if session['te_id'] == id:
        ...
        if request.method == 'POST':
            ...
            if who=="exam":
                ...
                flag= sessionDB.execute("select id from exam where course_code = :course and "
                                        "id = :examcode",{"course":course,"examcode":examcode}).fetchone()
...
def student(id):
    if session['student_id'] == id:
        ...
        #this is my query the problem is in total not defined we wiil replace total by it`s index
        s = sessionDB.execute("select studentcourses.grade,sum(studentexam.student_mark) as m,studentcourses.course_code from (select * from studentexam where studentexam.student_ID= :id) as studentexam inner join (select * from studentcourses where studentcourses.student_ID= :id) as studentcourses on studentexam.exam_course_code=studentcourses.course_code group by studentcourses.course_code;",{"id":id}).fetchall()
        ...
        if request.method == 'POST':
            ...
            if who == 'summer':
                ...
                if value:
                    ...
                    return render_template('studentpage.html', subjects=result_subject_test, summer=result_summer,
                                           final=result_final, score=result_score, id=id, alret="* Choose subjects",states =states, result=result)
            if who == 'drop':
...
def hours_calc(result_final, id):
    ...
    for subject in result_final:
        ...
        if subject.total >= temp.pass_mark:
